...
| --: | --- | --- |
| EPA Air Quality SYstem (AQS) | 2018-2019 | EPAair_O3_PM25_NC1819_Processed.csv | 
...
```{r data.summary, echo=F, warning=FALSE}
#Mean PM2.5 concentration for each county, sort by value
summary_PM25_CV <- epa_data %>% 
  drop_na(PM2.5) %>% 
  group_by(COUNTY) %>% 
  summarize(mean_PM2.5 = mean(PM2.5)) %>% 
  arrange(mean_PM2.5)

#Mean ozone concentration for each county, sort by value
summary_Ozone_CV <- epa_data %>% 
  drop_na(Ozone) %>% 
  group_by(COUNTY) %>% 
  summarize(mean_Ozone = mean(Ozone)) %>% 
  arrange(mean_Ozone)

#Produce the 2 tables 
kable(summary_PM25_CV,caption = "Mean Particulates (2.5mm)",col.names = c("County","$\\mu g/m^3$"))
kable(summary_Ozone_CV,caption = "Mean Ozone",col.names = c("County","$\\mu g/m^3$"))
```
...
```{r echo=FALSE, fig.cap="Ozone by Month in 2018", fig.align='left', warning=FALSE, fig.height=2.5}
plot_2018record <- epa_data %>% 
  filter(Year==2018) %>%
  ggplot(aes(x = as.character(Month),y = Ozone))+
  geom_boxplot()+
  labs(title ="Ozone Concentration by Month in 2018",x = "Month", y = "Ozone Concentration(µg/m3)") 

print(plot_2018record)
```

```{r echo=FALSE, fig.cap="Ozone by Month in 2019", fig.align='left', warning=FALSE, fig.height=2.5}
plot_2019record <- epa_data %>% 
  filter(Year==2019) %>%
  ggplot(aes(x = as.character(Month),y = Ozone) )+
  geom_boxplot() +
  labs(title ="Ozone Concentration by Month in 2019",x = "Month",y = "Ozone Concentration(µg/m3)")

print(plot_2019record)
```
...
https://github.com/ruiqingli0409/EDA-Spring2023/blob/185f3e08566630a08f7b43ee213891e0b1606f08/Assignments/A07_Lab_Crafting_Reports.pdf
