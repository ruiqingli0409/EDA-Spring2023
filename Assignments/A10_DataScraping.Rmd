...
water.system.name <- LWSP.Webpage %>% 
  html_nodes("div+ table tr:nth-child(1) td:nth-child(1) .q") %>% 
  html_text()
water.system.name
  
PWSID <- LWSP.Webpage %>% 
  html_nodes("td tr:nth-child(1) td+ td .q") %>% 
  html_text()
PWSID  
  
ownership <- LWSP.Webpage %>% 
  html_nodes("tr:nth-child(2) td+ td .q") %>% 
  html_text()
ownership

max.withdrawals.mgd  <- LWSP.Webpage %>% 
  html_nodes('.fancy-table:nth-child(31) th:nth-child(6)') %>%
  html_text()
max.withdrawals.mgd 
...
#4
df_LWSP <- data.frame("Month" = rep(1:12))
                             
df_LWSP <- df_withdrawals %>% 
