...
NTL.phys.data.Ex1 <- filter(NTL.phys.data, year4 == 1994)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Ex2 <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in%
                              c(1990:1999))
...
NTL.phys.data.Ex3 <- 
  NTL.phys.data%>%
  filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in%
                              c(1990:1999) & month(sampledate)== 7)

#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 
# through 1999 only for July.
NTL.phys.data.processed <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake" | lakename == "Peter Lake")
